     ...
     function deferredAddZip(url, filename, zip) {
        ...
        JSZipUtils.getBinaryContent(url, function (err, data) {
            if(err) {
                ...
                zip.file(filename,js, data, {binary:true});
...
jQuery(function ($) {
   ...
   /* function showError(text) {
    ...
    function deferredAddZip(url, filename, zip) {
        ...
        JSZipUtils.getBinaryContent(url, function (err, data) {
            if(err) {
                alert("deferredaddzip alyarm  " + err);
                deferred.reject(err);
            } else {
                alert("zipping file");
...
*/
    var $form = $("#download_form").on("submit", function () {
       ...
       // resetMessage();
        ...
        // section below doesn't work at all - alert used for simplest debugging
        $.when.apply($, deferreds).done(function () {
        	alert("must start download");
            ...
            alert(err);
            // Error: InvalidStateError: Failed to read the 'responseText' property from 'XMLHttpRequest': 
